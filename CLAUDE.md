...
## AgentView: model selection wired to the same Prefs/AIModel data model

- `AgentView.vue` had two bugs blocking this: `const agentPrepare = ref<DtoAgentPrepare>;` was missing its call/initial value (assigned the bare `ref` generic function, not an actual reactive ref), and its Model `<Select>` used a hardcoded `modelOptions` array (`Default`/`GPT-4o`/`GPT-4o-mini`) that didn't match the yaml-configured `app.openai.models.available` (`gpt-5`/`gpt-5-mini`/`gpt-4.1`). Fixed the ref init and pointed the `Select` at `agentPrepare.availableAIModels` with `optionLabel="label" optionValue="name"` — the same `AIModel(name, label)` shape/binding used in `Preferences.vue`.
- Backend support for this already existed on `DtoAgentPrepare` (`availableAIModels: Set<AIModel>`, `selectedAIModel: String`) but `AgentServiceImpl.prepareAssistant()` only populated them in the "new agent" branch — resuming an existing thread returned an empty model list. Added the same `preferencesService.aiModelsFromConfig(agentAvailableModels)` call to the "existing agent" branch too.
- `AgentView.vue`'s `prepareAgent()` now defaults `agentPrepare.model` from `selectedAIModel` when the backend doesn't set `model` directly (the new-agent case), so the dropdown isn't blank and the correct default model gets posted when the assistant is actually created.
